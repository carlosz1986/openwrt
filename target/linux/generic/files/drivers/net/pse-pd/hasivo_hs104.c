...
static void hs104_poll_work(struct work_struct *work)
{
	...
	/*
	 * LED triggers use hardware bit positions directly (not PSE PI
	 * indices), so use raw BIT(i) here — not HS104_PORT_BIT().
	 */
	for (i = 0; i < HS104_MAX_PORTS; i++) {
		struct hs104_port *port = &priv->ports[i];
		bool enabled = !!(pw_en & BIT(i));
		bool delivering = !!(pw_status & BIT(i));
...
/*
 * Sysfs interface for direct hardware register access (debug only).
 * Uses raw hardware bit positions. Bypasses PSE framework state tracking,
 * so changes here won't be reflected in ethtool --show-pse.
 */
...
static int hs104_init_triggers(struct hs104_priv *priv)
{
	...
	/* Use device tree node name for trigger naming to match DTS references */
	node_name = dev->of_node ? dev->of_node->name : dev_name(dev);

	for (i = 0; i < HS104_MAX_PORTS; i++) {
		struct hs104_port *port = &priv->ports[i];

		port->delivering = false;
		port->trig_delivering.name = devm_kasprintf(dev, GFP_KERNEL,
					"%s:port%ddelivering", node_name, i);
		if (!port->trig_delivering.name)
			return -ENOMEM;
		...
		port->trig_enabled.name = devm_kasprintf(dev, GFP_KERNEL,
					"%s:port%denabled", node_name, i);
		if (!port->trig_enabled.name)
			return -ENOMEM;
...
static int hs104_probe(struct i2c_client *client)
{
	...
	if (ret) {
		dev_err(dev, "Failed to register PSE controller: %d\n", ret);
		return ret;
	}
	...
	dev_info(dev, "HS104 PSE controller initialized (triggers: %s:port*)\n",
